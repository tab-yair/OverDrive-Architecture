...
void ThreadSafeFileManagement::create(const std::string& fileName, const std::string& content) {
    ...
    
    {
        ...
        innerManager->create(fileName, content);
    }
...
void ThreadSafeFileManagement::write(const std::string& fileName, const std::string& content) {
    auto fileMtx = getLock(fileName);
    
    {
        std::unique_lock<std::shared_mutex> lock(*fileMtx);
        innerManager->write(fileName, content);
    }
...
std::string ThreadSafeFileManagement::read(const std::string& fileName) {
    auto fileMtx = getLock(fileName);
    // Shared lock allows multiple readers for the same file
    std::shared_lock<std::shared_mutex> lock(*fileMtx);
    
    return innerManager->read(fileName);
}

void ThreadSafeFileManagement::remove(const std::string& fileName) {
...
bool ThreadSafeFileManagement::exists(const std::string& fileName) {
    auto fileMtx = getLock(fileName);
    // We need at least a shared_lock to ensure the file isn't being deleted *right now*
    std::shared_lock<std::shared_mutex> lock(*fileMtx);
    
    return innerManager->exists(fileName);
}
...
std::vector<std::string> ThreadSafeFileManagement::list() {
    // THREAD-SAFETY NOTE:
    // This operation deals only with listing file names and does not modify file contents.
    // We rely on the underlying IFileManagement to provide a consistent snapshot.
    // No file locks are acquired here because we're only reading file list, not touching individual files.
    ...
    return innerManager->list();
}

std::vector<std::string> ThreadSafeFileManagement::search(const std::string& content) {
    ...
    std::vector<std::string> fileNames = list();
    ...
    for (const auto& fileName : fileNames) {
        ...
        try {
            ...
            std::string fileContent = innerManager->read(fileName);
