...
TEST(ThreadSafeFileManagementTest, HeavyContentionOnSingleFile) {
    ...
    tfm.create(USER, SHARED_FILE, "INITIAL");
        ...
        threads.emplace_back([&, i]() {
            for (int j = 0; j < OPS_PER_THREAD; ++j) {
                try {
                    ...
                    tfm.write(USER, SHARED_FILE, data);
                    std::string readBack = tfm.read(USER, SHARED_FILE);
    ...
    EXPECT_TRUE(tfm.exists(USER, SHARED_FILE));
}
...
TEST(ThreadSafeFileManagementTest, ReadersWritersScenario) {
    ...
    tfm.create(USER, SHARED_FILE, "START");
    ...
    for(int i=0; i<10; ++i) {
        readers.emplace_back([&]() {
            while(running) {
                try {
                    std::string s = tfm.read(USER, SHARED_FILE);
    ...
    std::thread writer([&]() {
        for(int i=0; i<NUM_UPDATES; ++i) {
            try {
                tfm.write(USER, SHARED_FILE, "Update_" + std::to_string(i));
            } catch(...) {
                readErrors++;
            }
    ...
    std::string finalContent = tfm.read(USER, SHARED_FILE);
