...
private:
    ...
    using CommandCreator = std::function<std::unique_ptr<ICommand>(std::shared_ptr<ClientContext>)>;
...
public:
    ...
    std::map<std::string, std::unique_ptr<ICommand>> createCommands(std::shared_ptr<ClientContext> context) override;
};
